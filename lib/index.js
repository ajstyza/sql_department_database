...
function createDepartment() {
    ...
    function addRole(){
        ...
        .then((res) =>{
            db.query(`INSERT INTO roles (title, salary, department_id) VALUES ("${res.title}", ${res.salary}, ${res.department_id})" `)
        })
        ...
        inquirer.prompt([
            {
                type: 'input',
                names: 'employee',
                message: 'add first name, last name, and role id.'
            }
        ])
        .then((res) => {
            db.query(`INSERT INTO employees (first_name, last_name, role_id) VALUES ("${res.first_name}", "${res.last_name}", ${res.role_id})`)
        })
    };
